...
class ChatActivity : AppCompatActivity() {
    ...
    private const val AGORA_APP_ID = "79f72c03d10847dfaed960544afcf78a"
    }
    ...
    private fun startVoiceCall() {
        try {
            if (currentUid.isEmpty() || otherUid.isEmpty()) {
                Toast.makeText(this, "بيانات المستخدم غير كاملة", Toast.LENGTH_SHORT).show()
                return
            }

            val intent = Intent(this, VoiceCallActivity::class.java)
            intent.putExtra(VoiceCallActivity.EXTRA_APP_ID, AGORA_APP_ID)
            intent.putExtra(VoiceCallActivity.EXTRA_CHANNEL, chatId)
            intent.putExtra(VoiceCallActivity.EXTRA_OTHER_UID, otherUid.toIntOrNull() ?: 0)
            intent.putExtra(VoiceCallActivity.EXTRA_OTHER_NAME, otherName)
            intent.putExtra(VoiceCallActivity.EXTRA_CURRENT_UID, currentUid.hashCode())
            intent.putExtra(VoiceCallActivity.EXTRA_TOKEN, "")
            startActivity(intent)
        } catch (e: Exception) {
            Toast.makeText(this, "خطأ في بدء المكالمة: ${e.message}", Toast.LENGTH_SHORT).show()
        }
    }
}
